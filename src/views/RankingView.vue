  ...
  <div class="ranking-page">
    ...
    <div class="ranking-container">
      ...
      <div v-if="activeFilter === 'category'" class="category-selector">
        ...
        <!-- ✅ NUEVO: Debug info -->
        <div v-if="categories.length === 0" style="color: red; margin-top: 0.5rem;">
          ⚠️ No se cargaron categorías
        </div>
      </div>
...
// ============================================
onMounted(async () => {
  console.log('🎬 Montando RankingView...');
...
const loadCategories = async () => {
  try {
    ...
    if (Array.isArray(data)) {
      ...
      // Por si viene en un objeto
      categories.value = Object.values(data);
    } else {
      console.warn('⚠️ Formato inesperado de categorías:', data);
      categories.value = [];
    }
    
    console.log('✅ Categorías asignadas:', categories.value);
...
/**
 * ✅ Cargar ranking según filtro
 */
const loadRanking = async () => {
  loading.value = true;
  
  try {
    console.log('📊 Cargando ranking, filtro:', activeFilter.value);
    ...
    if (activeFilter.value === 'global') {
      ...
      console.log('✅ Ranking global obtenido:', ranking.value.length, 'usuarios');
      
      // Obtener posición del usuario
      if (currentUserId) {
        userPosition.value = await rankingService.getUserPosition(currentUserId);
        console.log('🏆 Posición del usuario:', userPosition.value);
      }
      
    } else if (activeFilter.value === 'category') {
      // Ranking por categoría
      if (selectedCategory.value) {
        console.log('📚 Obteniendo ranking de categoría:', selectedCategory.value);
        
        const data = await rankingService.getRankingByCategory(selectedCategory.value, 50);
        
        console.log('✅ Datos recibidos:', data);
        
        // ✅ Extraer ranking correctamente
        ranking.value = data.ranking || data || [];
        
        console.log('✅ Ranking por categoría:', ranking.value.length, 'usuarios');
      } else {
      ...
      const data = await rankingService.getActiveUsers(7, 50);
      ranking.value = data || [];
      
      console.log('✅ Usuarios activos:', ranking.value.length);
    }
